...
<head>
    ...
    <?php
        ...
        function verifica($ini, $fim, $points=1000){
            ...
            for ($z=$ini; $z <= $fim; $z += $precision) {
                $z = round($z, 6);
                // Loop para resolver a conta usando o valor de $z atual
                for($i=0; $i < count($_SESSION['dbases']); $i++){
                    $res += $_SESSION['dbases'][$i]*pow($z, $_SESSION['dexpoentes'][$i]);
                }
                ...
                if($res==0){
                    ...
                    if(($ant<0 && $res>0) || ($ant>0 && $res<0)){
                        ...
                        // echo " ASDF | " . $asdf . " | " . $res . "<br>";
                        array_push($_SESSION['m']['x'], $asdf);
                        array_push($_SESSION['m']['y'], $res);
                        // echo "ASDFFFF | <pre>";
                        // print_r($_SESSION['m']['x']);
                        // echo " | ";
                        // print_r($_SESSION['m']['y']);
                        // echo "</pre>";
                    }else $ant = $res;
                }else $ant = $res;
            }
        ...
        for ($x = $_GET['min']; $x <= $_GET['max']+0.001; $x+=0.001) {
            $res = 0;
            // Loop para resolver a conta usando o valor de X atual
        ...
        for ($x = $_GET['min']; $x <= $_GET['max']+0.001; $x+=0.001) {
            ...
            if($res==0){
                ...
                array_push($_SESSION['m']['y'], $res);
                //echo "ASDF | " . $_SESSION['m']['y'] . " | " . $ant . " | " . $res;
            }elseif(isset($ant)){
                if(($ant<0 && $res>0) || ($ant>0 && $res<0)){
                    // array_push($_SESSION['m']['x'], ($x + $x-0.001)/2);
                    // array_push($_SESSION['m']['y'], $res);
                    // echo "ASDF | " . $_SESSION['m']['x'][0] . " | " . $_SESSION['m']['y'][0] . "<br>";
                    //     echo "ASDFFFF | <pre>";
                    //     print_r($_SESSION['m']['x']);
                    //     echo " | ";
                    //     print_r($_SESSION['m']['y']);
                    ...
                    verifica($x-0.001, $x);
                    // echo "ASDF | " . $_SESSION['m']['x'][1] . " | " . $_SESSION['m']['y'][1] . "<br>";
                }
            }
            $ant = $res;
        }
